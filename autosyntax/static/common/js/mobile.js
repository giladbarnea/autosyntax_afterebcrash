...
function tablet_portrait() {
    ...
    let ham_home_mar_top;
    if (filename === 'cv') {
        ham_home_mar_top = -height / 35;
    }
    else {
        ham_home_mar_top = -height / 15;
    }
    hamburger_home(ml = '37%', mt = ham_home_mar_top);
    let fshamburger;
    let ham_menu_mt;
    let ham_menu_ml;
    if (filename === 'cv') {
        // fshamburger = height / 20 < width / 14 ? height / 20 : width / 14;
        fshamburger = smaller(height / 20, width / 14);
        let base = 110;
        if (height <= 420) {
            base += (420 - height) / 10;
        }
        // ham_menu_mt = '-110%';
        ham_menu_mt = '-' + base.toString() + '%';
        ham_menu_ml = '-95%';

    } else {
        fshamburger = smaller(height / 17, width / 14);
        ham_menu_mt = '-105%';
        ham_menu_ml = '-100%';
    }
...
function phone_landscape() {
    ...
    hamburger_home(ml = '32%', mt = ham_home_mar_top);
    pad_body(0);
    add_classes('pad_body', 'row-1');
    tops(_88 = '90%', _84 = '81%');
    _left('85%');
